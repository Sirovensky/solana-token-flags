...
A daily, machine-readable list of tokens in the top Meteora DLMM pools **and Jupiter's 200 most-traded tokens** that carry **Token-2022 transfer taxes or issuer controls**: transfer fee, permanent delegate (issuer can pull tokens from any wallet), pausable mint, transfer hook, plus whether mint and freeze authority are still live.
...
GitHub Actions, daily at 06:17 UTC (`.github/workflows/daily.yml`). `scan.py` needs only `requests`; run it locally with `SOLANA_RPC=<your rpc> python3 scan.py`. Scans up to 600 DLMM pools with TVL ≥ $20k ordered by 24h volume plus Jupiter's top-traded list, and reads every mint with `getMultipleAccounts`. Each entry carries a `source` list (`meteora_dlmm`, `jupiter_top_traded`) and, where available, `jup_volume_24h`.
